...
void compileFuncDecl(void) {
  ...
  if(scope->owner->funcAttrs->ReturnFunc == 0 ){
    error(ERR_NORETURNFUNCTION , lookAhead->lineNo, lookAhead->colNo);
  }
...
Type* compileLValue(void) {
  ...
  if (var->kind == OBJ_VARIABLE) {
    ...
    if(scope->owner->kind == OBJ_FUNCTION && strcmp(scope->owner->name, currentToken->string) == 0)
    scope->owner->funcAttrs->ReturnFunc += 1;
  }
...
void compileAssignSt(void) {
  ...
  Type *LType[15], *RType[15];
  int Lcounter = 0, Rcounter = 0;
  int isString = 0;
  Scope* scope = symtab->currentScope;
  int tmp = scope->owner->funcAttrs->ReturnFunc ;
  do {
    if(lookAhead->tokenType == SB_COMMA) eat(SB_COMMA);
    LType[Lcounter] = compileLValue();
    if( LType[Lcounter]->typeClass == TP_ARRAY && LType[Lcounter]->elementType->typeClass == TP_CHAR){
      isString = 1;
    }
    Lcounter++;
    ...
    case SB_ASSIGN_PLUS:
      eat(SB_ASSIGN_PLUS);
      if( isString){
        error( ERR_OPERATOR_ASSIGN_WITH_CHAROFSTRING, currentToken->lineNo, currentToken->colNo);
      }
      if(scope->owner->funcAttrs->ReturnFunc > 0)
        scope->owner->funcAttrs->ReturnFunc = tmp;
      break;
  ...
  do {
    if(lookAhead->tokenType == SB_COMMA) eat(SB_COMMA);
    RType[Rcounter] = compileExpression();
    Rcounter++;
  } while(lookAhead->tokenType == SB_COMMA);
  if(Lcounter != Rcounter) {
    printf("Number of Left and Right of AssigntSt not equal: %d - %d\n", currentToken->lineNo, currentToken->colNo);
    exit(1);
  } else {
    int i; 
...
void compileForSt(void) {
  ...
  checkForIndexType(var->varAttrs->type);
  checkIntType(var->varAttrs->type);

  eat(SB_ASSIGN);
  type = compileExpression();
  checkForIndexType( type);
  checkIntType( type);
...
void compileArgument(Object* param) {
  ...
  if(param->paramAttrs->kind == PARAM_REFERENCE) {
    if(lookAhead->tokenType == TK_IDENT) { // passing a reference
...
Type* compileIndexes(Type* arrayType) {
  ...
  while (lookAhead->tokenType == SB_LEFT_BRACKET) {
    eat(SB_LEFT_BRACKET);
