...
Token* readNumber(void) {
  ...
  int isError = 0;
  string[ index++] = (char)currentChar;
  readChar();
  while( (currentChar >= (int)'0' && currentChar <= (int)'9') || currentChar == (int)'.'){
    string[ index++] = (char)currentChar;
    if( currentChar == (int)'.'){
      if( type == TK_NUMBER){
        type = TK_FLOAT;
...
Token* readString(void) {
  ...
  while( charCodes[ currentChar] != CHAR_DOUBLEQUOTE){
    ...
    if( currentChar == '\n' || currentChar == EOF){
      token = makeToken(TK_NONE, ln, cn);
      error(ERR_EXPECTED_ENDSTRING, ln, cn);
      isError = 1;
      break;
    }
    readChar();
...
Token* getToken(void) {
  ...
  case CHAR_LEFT_BRACKET:
    token = makeToken(SB_LEFT_BRACKET,lineNo,colNo);
    readChar();
    return token;
  case CHAR_RIGHT_BRACKET:
    token = makeToken(SB_RIGHT_BRACKET,lineNo,colNo);
    readChar();
    return token;
  default:
...
void printToken(Token *token) {
  ...
  case SB_LEFT_BRACKET : printf("SB_LEFT_BRACKET\n"); break;
  case SB_RIGHT_BRACKET : printf("SB_RIGHT_BRACKET\n"); break;
  }
}
